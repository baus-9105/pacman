...
Project 1 – Hide and Seek Arenaaims to help students understand, implement, and optimize
search algorithms through an automated arena that simulates the game of hide-and-seek, where
student teams can compete against one another for ranking.
...
The Arena is an automated framework, implemented in Python, that serves as the competitive environment for the agents. Given the teams’ submissions, the Arena will (1) automatically import each team’s agents; (2) run a series of matches between teams; and (3) output the results.

**Algorithm 1** Agent – Environment Interaction Loop
```
while episode not terminateddo
ap←PacmanAgent.step()
ag←GhostAgent.step()
...
Note: Both agents receive the same state information and make their decisions simultaneously.  
...
Please note that, the framework is provided to you for reference, evaluting your own agent against the above conditions may require some tweakings.
...
```
```
Important note: If both agents move at the same speed, the Seek Agent can never win
because it cannot make contact with the Hide Agent. To ensure the fairness, the Seek Agent
moves at a speed of 2 cells per step when moving in straight line but cannot move with an L-
shaped immediately during a turn.
...
```
The recommended algorithms are summarized in Table 1.
```
```
Table 1: Suggested Algorithms for Seek and Hide Agents
Seek Agent Hide Agent
```
...
- Greedy
- etc.
    - BFS: finding the farthest position
    - Minimax/Expectimax
    - Monte Carlo
    - Potential Fields
    - etc.
...
```
winratehide=Hide gamesHide wins winrateseek=Seek gamesSeek wins
```
```
The results are recorded in the following example table.
```
```
Table 2: Match Results Between Teams (Hide vs. Seek)
Hide\Seek Team 1 Team 2 Team 3 Team 4
Team 1 – 0 1 1
Team 2 0 – 1 0
Team 3 1 0 – 1
Team 4 0 0 1 –
...
Based on Table 2, an example of the win rates for Team 1 is as follows:
```
- As the Hide agent, Team 1 wins 1 out of 3 matches, resulting in winratehide= 33.3%.
- As the Seek agent, Team 1 wins 2 out of 3 matches, resulting in winrateseek= 66.6%.
...
Each team must submit a single.zipfile to Moodle containing the following directory structure
wheregroupiddenotes a single integer for your group’s ID only:
...
Note:Late submissions willnotbe accepted under any circumstances.
...
- This is aGROUPassignment. Each group has 3 - 4 members.
...
- Student teams must ensure that their agent returns an action within a maximum of1 second.
    The built-in"time"library may be used together with all built-in libraries bundled with
    Python 3. External libraries available for your team includes: numpy, pandas, scipy, and
    gurobi.
- AI tools areNOT restricted; however, students should use them wisely. Lab instructors
